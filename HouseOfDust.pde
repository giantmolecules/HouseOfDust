...
int quatrainSpace = 170;

void setup() {
  size(1500, 1800, PDF, "HouseOfDust.pdf");
  textMode(SHAPE);
  textSize(textSize);
  background(255);
  fill(0);
...
void draw() {
  ...
  for (int i = 0; i < 1; i++) {
    for (int q = 0; q < 20; q++) {
      ...
      text("A house of "+materials[randomInt], originX, originY+quatrainSpace*q);
      ...
      text(locations[randomInt], originX+spacingX, originY+spacingY+quatrainSpace*q);
      ...
      text(lighting[randomInt], originX+spacingX*2, originY+spacingY*2+quatrainSpace*q);
      ...
      text(inhabitants[randomInt], originX+spacingX*3, originY+spacingY*3+quatrainSpace*q);
    }
  }
  exit();
}